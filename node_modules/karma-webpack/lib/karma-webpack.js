...
/* config.colors */colors, customFileHandlers, emitter) {
  webpackOptions = clone(webpackOptions);
  webpackMiddlewareOptions = clone(webpackMiddlewareOptions || webpackServerOptions);
...
/*********************************************************************************
 * Karma Webpack as a framework plugin
 *********************************************************************************/

const KarmaWebpack = require('./KarmaWebpack');

function fakeRimraf(dir_path) {
  if (fs.existsSync(dir_path)) {
    fs.readdirSync(dir_path).forEach(function(entry) {
      var entry_path = path.join(dir_path, entry);
      if (fs.lstatSync(entry_path).isDirectory()) {
        fakeRimraf(entry_path);
      } else {
...
const controller = new KarmaWebpack();

function initKarmaWebpack(files, config, emitter, preprocess, fileList) {
  console.log('BOOOOOMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMM');
  // fakeRimraf(path.join(config.basePath, '.karma-cache/'));

  files.unshift({ pattern: path.join(config.basePath, '.karma-cache/commons.bundle.js'), included: true, served: true, watched: false });
  files.unshift({ pattern: path.join(config.basePath, '.karma-cache/runtime.bundle.js'), included: true, served: true, watched: false });

  // files.push({ pattern: path.join(config.basePath, '.karma-cache/*.js'), included: true, served: true, watched: false });
  files.push({ pattern: path.join(config.basePath, '.karma-cache/foo-one.test.bundle.js'), included: true, served: true, watched: false });
  files.push({ pattern: path.join(config.basePath, '.karma-cache/foo-two.test.bundle.js'), included: true, served: true, watched: false });
}

function preprocessorFactory(config, emitter) {
  const { files, preprocessors } = config;
...
initKarmaWebpack.$inject = ['config.files', 'config', 'emitter', 'preprocess', 'fileList'];
preprocessorFactory.$inject = ['config', 'emitter'];

module.exports = {
  'preprocessor:webpack': ['factory', preprocessorFactory],
  'framework:webpack': ['factory', initKarmaWebpack]
}
