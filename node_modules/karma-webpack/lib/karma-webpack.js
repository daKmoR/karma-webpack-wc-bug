...
function initKarmaWebpack(config) {
  // fakeRimraf(path.join(config.basePath, '.karma-cache/'));

  config.files.unshift({ pattern: path.join(config.basePath, '.karma-cache/commons.js'), included: true, served: true, watched: false });
  config.files.unshift({ pattern: path.join(config.basePath, '.karma-cache/runtime.js'), included: true, served: true, watched: false });
}
...
initKarmaWebpack.$inject = ['config'];
